...
import apacheMD5 from "apache-md5"; // Použij apache-md5, který je dostupný na npm!
...
export async function loginToWebshare(username, password) {
    ...
    // Použij apache-md5 s přesným solem "webshare"
    const md5hash = apacheMD5(password, "webshare");
    console.log("Webshare login - md5crypt hash:", md5hash);

    // Výsledek md5crypt zahashuj přes sha1
    const finalHash = sha1(md5hash);
    console.log("Webshare login - sha1(md5crypt) hash:", finalHash);

    // Připrav POST data a hlavičky dle Webshare API
