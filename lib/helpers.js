...
import md5crypt from "md5-crypt"; // Správný balíček pro MD5 crypt s prefixem $1$
...
export async function loginToWebshare(username, password) {
    ...
    // Správné hashování: md5crypt s prefixem $1$ a solem "webshare"
    const md5hash = md5crypt(password, "$1$webshare");
