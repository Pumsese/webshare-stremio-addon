...
import md5 from "nano-md5"; // npm install nano-md5
...
export async function loginToWebshare(username, password) {
    // 1. Zkus hash variantu (SHA1(md5crypt))
    console.log("Webshare login (hash variant) - username:", username);
    const md5hash = md5.crypt(password, "$1$webshare$");
    console.log("Webshare login (hash variant) - md5crypt hash:", md5hash);

    const finalHash = sha1(md5hash);
    console.log("Webshare login (hash variant) - sha1(md5crypt) hash:", finalHash);

    let postBody = `username_or_email=${encodeURIComponent(username)}&password=${finalHash}&keep_logged_in=1`;
    console.log("Webshare login (hash variant) - POST body:", postBody);

    let res = await fetch("https://webshare.cz/api/login/", {
        method: "POST",
        headers: {
    ...
    console.log("Webshare login (hash variant) - HTTP status:", res.status);
    console.log("Webshare login (hash variant) - response XML:", xml);

    let statusMatch = xml.match(/<status>(.*?)<\/status>/s);
    let tokenMatch = xml.match(/<token>(.*?)<\/token>/s);

    if (statusMatch && statusMatch[1] === "OK" && tokenMatch) {
        // Úspěch s hash variantou
    ...
    // 2. Pokud hash varianta selže, zkus čisté heslo (Coffei varianta)
    console.log("Webshare login (plain variant) - fallback na čisté heslo");

    postBody = `username_or_email=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}&keep_logged_in=1`;
    console.log("Webshare login (plain variant) - POST body:", postBody);

    res = await fetch("https://webshare.cz/api/login/", {
        method: "POST",
        headers: {
            "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
            "Accept": "text/xml; charset=UTF-8",
    ...
    xml = await res.text();
    console.log("Webshare login (plain variant) - HTTP status:", res.status);
    console.log("Webshare login (plain variant) - response XML:", xml);

    statusMatch = xml.match(/<status>(.*?)<\/status>/s);
    tokenMatch = xml.match(/<token>(.*?)<\/token>/s);
    ...
    // Pokud selžou obě varianty, vypiš chybu
